...
contract TuringAlpacaFarmBNB is Ownable {
        ...
        bool public isEnableEmergencyWithdrawibbnb = false;
        ...
        event onEmergencyWithdrawibbnb(address _user, uint256 _amount);
        constructor (
        ...
        */
        ...
        function openEmergencyWithdrawibbnb() public onlyOwner {
                isEnableEmergencyWithdrawibbnb = true;
        }
        function stopEmergencyWithdrawibbnb() public onlyOwner {
                isEnableEmergencyWithdrawibbnb = false;
        }
        function withdrawibbnbFromAlpaca() public onlyOwner {
        ...
        */
        ...
        function withdraw(uint256 _amount) external inNetworkEnable onlyWhitelist 
        {
                ...
                uint256 _fee = getFarmFee();
                _withdrawFromAlpaca();
                uint256 _availableBal = availableBal();
                if (_availableBal < _amount) {
                        _amount = _availableBal;
                }
        ...
        function emergencyWithdrawibbnb() external onlyWhitelist {
                uint256 _shares = shareOf[msg.sender];
                require(isEnableEmergencyWithdrawibbnb == true, 'TUR_FARM:INVALID_PERMISSION');
                require(_shares  > 0, 'TUR_FARM:INVALID_USER_SHARE');
                uint256 _ibBNBBal = ibBNB.balanceOf(address(this));
                require(_ibBNBBal  > 0, 'TUR_FARM:INVALID_IBBNB_BAL');
                // Get ibBNB will transfer for user.
                uint256 _amountOut = _shares.mul(_ibBNBBal).div(totalShare);
                require(_ibBNBBal  >= _amountOut, 'TUR_FARM:INVALID_AMOUNT_OUT');
                // Clear user's share
                shareOf[msg.sender] = 0;
                totalShare = totalShare.sub(_shares, 'TUR_FARM:INVALID_TOTAL_SHARE');
                // transfer ibBNB to user
                ibBNB.transfer(msg.sender, _amountOut);
                emit onEmergencyWithdrawibbnb(msg.sender, _amountOut);
        }
        /**
        * *************************************************************************************
        * INTERNAL ACTION
        ...
        */
        function getTotalBal() public view returns(uint256) 
        {
                ...
                return availableBal().add(_totalIbBNBValue).add(_totalAlpacaValue).sub(_alpacaFee).sub(_supplyFee);
        }
        ...
        function getData(address _user) 
                public 
                view
                returns(
                        uint256 bnbPrice_,
        ...
        function getTuringApy() public view returns(uint256) 
        {
                ...
                if (_totalSupplyValue <= 0) {
                        _totalSupplyValue = availableBal();
                }
                if (_totalSupplyValue <= 0) {
                        return 0;
                }
