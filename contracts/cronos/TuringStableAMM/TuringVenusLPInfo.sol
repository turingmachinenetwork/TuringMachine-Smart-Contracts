...
contract TuringVenusLPInfo {
	...
	function getData(address _owner, ITuringVenusLP[] calldata _lps) public view returns(LP[] memory data_)
	{
		...
		for (uint256 idx = 0; idx < _lps.length; idx++) {
			...
			data_[idx].uBaseAllowed = IBEP20(_base).allowance(_owner, address(_lps[idx]));
			data_[idx].uTokenAllowed = IBEP20(_token).allowance(_owner, address(_lps[idx]));
		}
	}
}
