...
contract TuringNoLossLotteryCakeV2 is ReentrancyGuard, Ownable {
    ...
    event onDraw(uint256 _index, address _user, uint256 _amount, uint256 _performanceFee, uint256 _cakeConvertToTuringAmt);
    ...
    function approveConnectToPancake() public {
        ...
        want.approve(address(pancakeSwap), uint256(-1));
    }
    ...
    function setRateOfConvertWinAmtToTuring() public onlyOwner {
        TimeLock storage _timelock = timeLockOf[keccak256(abi.encode('setRateOfConvertWinAmtToTuring'))];

        _validateTimelock(_timelock);
        require(_timelock.uintOf[keccak256(abi.encode('rateOfConvertWinAmtToTuring'))] >= 0, "INVALID_AMOUNT");
        require(_timelock.uintOf[keccak256(abi.encode('rateOfConvertWinAmtToTuring'))] <= 10000, "INVALID_AMOUNT");

        rateOfConvertWinAmtToTuring = _timelock.uintOf[keccak256(abi.encode('rateOfConvertWinAmtToTuring'))];
        delete _timelock.uintOf[keccak256(abi.encode('rateOfConvertWinAmtToTuring'))];
        _timelock.queuedTransactions = false;
    }
    ...
    function draw() external nonReentrant {
        ...
        _wonAmt = _wonAmt.sub(_performanceFee);

        uint256 _cakeConvertToTuringAmt = _wonAmt.mul(rateOfConvertWinAmtToTuring).div(10000);
        uint256 _turRewardAmt = 0;
        if (_cakeConvertToTuringAmt > 0) {
            _turRewardAmt = _transferTuringReward(_userLucky, _cakeConvertToTuringAmt);
        }

        _wonAmt = _wonAmt.sub(_cakeConvertToTuringAmt);
        ...
        emit onDraw(_tiketsWin, _userLucky, _wonAmt, _performanceFee, _cakeConvertToTuringAmt);
    }
    ...
    function _transferTuringReward(address _user, uint256 _amount) private returns(uint256)
    {
        ...
        pancakeSwap.swapExactTokensForTokens(_amount, 0, path, address(this), block.timestamp);
